...
const size_t NONCE_SIZE
    = crypto_secretbox_NONCEBYTES; // XSalsa20 nonce size (24 bytes)
const size_t KEY_SIZE
    = crypto_secretbox_KEYBYTES; // XSalsa20 key size (32 bytes)
...
public:
  ...
  APKGWriterV1 (const std::string &p, const std::string &sig = "SIGNATURE",
                const std::string &pwd = "")
      : path (p), dev_sig (sig), password (pwd)
  {
  }
  ...
  void
  add_file (const std::string &filepath, const std::string &arcname = "")
  {
    ...
    for (auto &file : files)
      {
        ...
        file_table.insert (file_table.end (), (uint8_t *)&name_len,
                           (uint8_t *)&name_len + 4);
        file_table.insert (file_table.end (), file.name.begin (),
                           file.name.end ());

        uint64_t offset = current_offset;
        uint64_t size = file.data.size ();
        file_table.insert (file_table.end (), (uint8_t *)&offset,
                           (uint8_t *)&offset + 8);
        file_table.insert (file_table.end (), (uint8_t *)&size,
                           (uint8_t *)&size + 8);

        file_data.insert (file_data.end (), file.data.begin (),
                          file.data.end ());
        current_offset += size;
      }
    ...
    if (!password.empty ())
      {
        ...
        if (crypto_pwhash (
                key.data (), KEY_SIZE, password.c_str (), password.size (),
        ...
        std::vector<uint8_t> cipher (block.size ()
                                     + crypto_secretbox_MACBYTES);
        if (crypto_secretbox_easy (cipher.data (), block.data (),
                                   block.size (), nonce.data (), key.data ())
            != 0)
          {
        ...
        header_extra.insert (header_extra.end (), (uint8_t *)&SALT_SIZE,
                             (uint8_t *)&SALT_SIZE + 4);
        header_extra.insert (header_extra.end (), salt.begin (), salt.end ());
        header_extra.insert (header_extra.end (), (uint8_t *)&NONCE_SIZE,
                             (uint8_t *)&NONCE_SIZE + 4);
        header_extra.insert (header_extra.end (), nonce.begin (),
                             nonce.end ());
...
public:
  ...
  APKGReaderV1 (const std::string &p, const std::string &password = "")
      : path (p)
  {
    ...
    if (file_table_offset > in.tellg ())
      {
        ...
        in.read (reinterpret_cast<char *> (header_extra.data ()),
                 header_extra_len);
      }

    // Read the remaining block (file table + data)
    std::vector<uint8_t> block ((std::istreambuf_iterator<char> (in)),
                                std::istreambuf_iterator<char> ());
    ...
    if (flags & FLAG_ENCRYPTED)
      {
        ...
        size_t ptr = 0;
        uint32_t salt_len
            = *reinterpret_cast<uint32_t *> (header_extra.data () + ptr);
        ptr += 4;
        std::vector<uint8_t> salt (header_extra.begin () + ptr,
                                   header_extra.begin () + ptr + salt_len);
        ptr += salt_len;

        uint32_t nonce_len
            = *reinterpret_cast<uint32_t *> (header_extra.data () + ptr);
        ptr += 4;
        std::vector<uint8_t> nonce (header_extra.begin () + ptr,
                                    header_extra.begin () + ptr + nonce_len);
        ...
        // Decrypt the block
        std::vector<uint8_t> decrypted (block.size ()
                                        - crypto_secretbox_MACBYTES);
        if (crypto_secretbox_open_easy (decrypted.data (), block.data (),
                                        block.size (), nonce.data (),
                                        key.data ())
            != 0)
          {
            throw std::runtime_error (
                "Decryption failed or archive tampered with");
          }
    ...
    for (uint32_t i = 0; i < file_count; ++i)
      {
        uint32_t name_len
            = *reinterpret_cast<uint32_t *> (block.data () + ptr);
        ptr += 4;
        std::string name (reinterpret_cast<char *> (block.data () + ptr),
                          name_len);
        ptr += name_len;
  ...
  read_file (const std::string &filename)
  {
    for (const auto &f : files)
      {
        if (f.name == filename)
          return std::vector<uint8_t> (data_block.begin () + f.offset,
                                       data_block.begin () + f.offset
                                           + f.size);
      }
  ...
  void
  extract_all (const std::string &outdir)
  {
    std::filesystem::create_directories (outdir);
    for (const auto &f : files)
      {
        std::filesystem::path outpath
            = std::filesystem::path (outdir) / f.name;
        std::filesystem::create_directories (outpath.parent_path ());
        std::ofstream out (outpath, std::ios::binary);
        out.write (
            reinterpret_cast<const char *> (data_block.data () + f.offset),
            f.size);
      }
  }
